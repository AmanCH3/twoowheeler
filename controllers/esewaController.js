...
/**
 * NEW: This controller is designed for the NATIVE MOBILE SDK flow.
 * It does NOT generate a signature. It simply provides the necessary
 * transaction details to the mobile app.
 */
const initiateEsewaPaymentApp = async (req, res) => {
  try {
    const { bookingId } = req.body;
    const booking = await Booking.findById(bookingId);
    ...
    res.json({
      success: true,
      message: 'Payment initiation details retrieved successfully',
      data: paymentDetails, 
    });
